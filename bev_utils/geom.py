import torch
from .basic import pack_seqdim, unpack_seqdim, matmul2 #, get_list_stats
...
def voxelize_lidar_pts(vox_util, pts, rots, trans, compress=True, device='cuda'):

    B = pts.shape[0]

    # Transform and prepare point cloud data
    xyz_velo0 = pts.to(device).permute(0, 2, 1)  

    # # Calculate the magnitude of the 3D velocities in the point cloud data
    mag = torch.norm(xyz_velo0, dim=2)
    xyz_velo0 = xyz_velo0[:,mag[0]>1]           # Filter out points with low magnitude
    xyz_velo0_bak = xyz_velo0.clone()

    # # Prepare camera and lidar transformation matrices
    velo_T_cams = merge_rtlist(rots, trans).to(device)

    __p = lambda x: pack_seqdim(x, B)
    __u = lambda x: unpack_seqdim(x, B)
    cams_T_velo = __u(safe_inverse(__p(velo_T_cams)))
    # # Transform point cloud data to camera coordinates
    xyz_cam0 = apply_4x4(cams_T_velo[:,0], xyz_velo0)

    # # Voxelize camera-based point cloud data
    occ_mem0 = vox_util.voxelize_xyz(xyz_cam0, 200, 8, 200, assert_cube=False)
    
    # B x 1 x Z x Y x X
    occ_mem0 = occ_mem0.permute(0, 1, 3, 2, 4)
    #print('occ mem 0 ', occ_mem0.shape)
    if compress:
        occ_mem0 = occ_mem0.sum(dim=2) #.unsqueeze(1)
    
        occ_mem0[occ_mem0>0] = 1 

    else:
        occ_mem0 = occ_mem0.squeeze(1)
