...
from .geom import * #eye_4x4, safe_inverse, apply_4x4, get_camM_T_camXs, split_rt_single, split_rt, merge_rt, merge_lrt
from .basic import * 
...
class Vox_util(object):
    ...
    def Ref2Mem(self, xyz, Z, Y, X, assert_cube=False):
        ...
        xyz = apply_4x4(mem_T_ref, xyz) #geom.
        return xyz
    ...
    def Mem2Ref(self, xyz_mem, Z, Y, X, assert_cube=False):
        ...
        xyz_ref = apply_4x4(ref_T_mem, xyz_mem) # geom.
        return xyz_ref
    ...
    def get_mem_T_ref(self, B, Z, Y, X, assert_cube=False, device='cuda'):
        ...
        center_T_ref = eye_4x4(B, device=device) # geom.
        ...
        mem_T_center = eye_4x4(B, device=device) #geom.
        mem_T_center[:,0,0] = 1./vox_size_X
        mem_T_center[:,1,1] = 1./vox_size_Y
        mem_T_center[:,2,2] = 1./vox_size_Z
        mem_T_ref = matmul2(mem_T_center, center_T_ref) # basic.
    ...
    def unproject_image_to_mem(self, rgb_camB, pixB_T_camA, camB_T_camA, Z, Y, X, assert_cube=False, xyz_camA=None):
        ...
        if xyz_camA is None:
            xyz_memA = gridcloud3d(B, Z, Y, X, norm=False, device=pixB_T_camA.device) # basic.
            xyz_camA = self.Mem2Ref(xyz_memA, Z, Y, X, assert_cube=assert_cube)

        xyz_camB = apply_4x4(camB_T_camA, xyz_camA) #geom.
        z = xyz_camB[:,:,2]

        xyz_pixB = apply_4x4(pixB_T_camA, xyz_camA) # geom.
        ...
        #         values[b] = bilinear_sample_single(rgb_camB[b], x_pixB[b], y_pixB[b]) # samp.
        # else:
        # native pytorch version
        y_pixB, x_pixB = normalize_grid2d(y, x, H, W) # basic.
        ...
        # INSTEAD OF SAMPLING, USE LIDAR INFO HERE?
    ...
    def warp_tiled_to_mem(self, rgb_tileB, pixB_T_camA, camB_T_camA, Z, Y, X, DMIN, DMAX, assert_cube=False):
        ...
        z_tileB, y_pixB, x_pixB = normalize_grid3d(z_tileB, y, x, D, H, W) # basic.
    ...
    def xyz2circles(self, xyz, radius, Z, Y, X, soft=True, already_mem=True, also_offset=False, grid=None):
        ...
        if grid is None:
            grid_z, grid_y, grid_x = meshgrid3d(B, Z, Y, X, stack=False, norm=False, device=xyz.device) # basic.
    ...
    def xyz2circles_bev(self, xyz, radius, Z, Y, X, already_mem=True, also_offset=False):
        ...
        grid_z, grid_x = meshgrid2d(B, Z, X, stack=False, norm=False, device=xyz.device) # basic.
        # note the default stack is on -1
